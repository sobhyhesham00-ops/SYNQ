...
if (!code.includes("activeTab === 'profile'")) {
  const replacement = `{activeTab === 'profile' && currentUser && (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <div className="bg-white/5 border border-white/10 rounded-2xl p-5 relative overflow-hidden shadow-inner flex flex-col h-full">
                        ...
                        <form className="flex flex-col gap-2 mb-4" onSubmit={(e) => {
                           ...
                           const minutes = parseInt(fd.get('mins')) || 0;
                           if (!text) return;
              ...
              {activeTab === 'dashboard' && (() => {`;
  code = code.replace(/\{activeTab === 'dashboard' && \(\(\) => \{/, replacement);
  fs.writeFileSync('src/App.tsx', code);
  console.log('Injected profile successfully.');
} else {
  console.log('Profile already injected.');
}
