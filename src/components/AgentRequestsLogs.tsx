    ...
    primaryContent = (
      <div className="space-y-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {req.platform && (
            <span className="px-2.5 py-1 rounded-xl text-[11px] font-bold uppercase tracking-widest bg-slate-500/10 text-slate-300 border border-transparent flex items-center gap-1.5">
              🌐 {req.platform}
            </span>
          )}
          {req.customerType && (
            <span className={`px-2.5 py-1 rounded-xl text-[11px] font-bold uppercase tracking-widest border flex items-center gap-1.5 ${ req.customerType === 'new' ? 'bg-teal-500/10 text-teal-400 border-transparent' : 'bg-purple-500/10 text-purple-400 border-purple-500/20' }`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-white/5 p-3 rounded-xl border border-white/8">
          ...
          {req.fileNumber ? (
            <div><p className="text-[11px] uppercase text-slate-500 font-semibold tracking-wider">📁 File Number</p><p className="text-xs text-amber-300 mt-0.5 font-mono font-bold">{req.fileNumber}</p></div>
          ) : (
            <div><p className="text-[11px] uppercase text-slate-500 font-semibold tracking-wider">📂 Customer Type</p><p className="text-xs text-slate-300 mt-0.5">New Customer (No File)</p></div>
          )}
        </div>
        <div>
          <p className="text-[11px] uppercase text-slate-500 font-semibold tracking-wider">💬 Inquiry Description</p>
          <p className="text-xs text-slate-200 mt-0.5 whitespace-pre-wrap break-words bg-transparent p-2.5 rounded-xl border border-white/8">{req.text}</p>
        </div>
        {req.answer && (
          <div className="bg-emerald-500/5 p-3 rounded-xl border border-emerald-500/10">
            <p className="text-[11px] uppercase text-emerald-500 font-semibold tracking-wider">✅ Answer / Resolution</p>
            <p className="text-xs text-emerald-200 mt-0.5 whitespace-pre-wrap break-words">{req.answer}</p>
          </div>
        )}
      </div>
    );
        ...
        <div className="flex items-center gap-1">
          {canShowEdit ? (
            <>
              <button 
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-white/5 hover:bg-white/10 text-xs font-semibold text-slate-300 transition-colors cursor-pointer"
              >
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 bg-rose-500/5 border border-rose-500/10 rounded-xl">
              <span className="text-[11px] text-rose-400/80 font-mono tracking-wider uppercase font-semibold flex items-center gap-1">
                 <Clock className="w-3 h-3" />
                 Editing period expired {expiryTimeStr && `at ${expiryTimeStr}`}
              </span>
              <span className="text-[11px] text-slate-400 border-l border-white/8 pl-2">Use</span>
              <button 
                onClick={() => document.getElementById(`reply-input-${req.id}`)?.focus()}
                className="text-[11px] text-sky-400 hover:text-sky-300 hover:underline font-bold transition-colors cursor-pointer flex items-center gap-1"
              >
                Reply / Add Update
              </button>
            </div>
          ...
          {isCancellable && (
            <button 
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-rose-500/10 hover:bg-rose-500/10 text-xs font-semibold text-rose-400 transition-colors cursor-pointer"
            >
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-xs text-left mt-1">Review status, details and track tickets effectively in your workspace.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white/[0.04] border border-white/8 p-4 rounded-xl flex flex-col items-center justify-center">
          ...
          <span className="text-[11px] text-indigo-500/70 uppercase tracking-widest font-bold mt-1">Matched</span>
        </div>
      ...
      <div className="bg-slate-950 border border-white/8 rounded-xl p-5 space-y-4">
        ...
        {isNoFiltersActive && (
          <div className="bg-transparent border border-white/12 text-white border border-transparent text-indigo-400 px-4 py-2 rounded-xl text-xs font-medium mb-4 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-3 pb-4 border-b border-white/8">
          <div className="relative w-full xl:col-span-2">
            ...
            <input type="text" placeholder="Search by ID, Name..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-1.5 bg-slate-900/60 border border-white/8/40 rounded-xl text-xs text-white focus:outline-none focus:border-indigo-500 font-sans" />
          </div>
        ...
        <div className="space-y-4 max-h-[calc(100vh-20rem)] overflow-y-auto pr-2">
          {sorted.length === 0 ? (
            <div className="text-center py-12 text-slate-400 space-y-3">
              ...
              <p className="text-xs">No requests found matching your filter.</p>
            </div>
          ) : (
            sorted.map((req) => {
