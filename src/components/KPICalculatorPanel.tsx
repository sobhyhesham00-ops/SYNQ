...
export const KPICalculatorPanel = () => {
  ...
  const inputClass = 'w-full bg-white/5 border border-white/8 rounded-xl px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-indigo-500 font-sans font-bold';
  ...
  const KPIResult = ({ label, value, unit, color, target }: any) => (
    <div className={`bg-transparent border ${color} rounded-xl p-4 flex flex-col gap-1`}>
      ...
      <p className='text-3xl font-bold text-white font-mono'>{value ?? '—'}{value ? <span className='text-lg text-slate-400 ml-1'>{unit}</span> : null}</p>
