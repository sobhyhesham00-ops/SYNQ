...
export const KPICalculatorPanel = () => {
  ...
  const inputClass = 'w-full bg-white/5 border border-white/8 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-indigo-500 font-sans font-bold';
  const labelClass = 'block text-[11px] text-slate-400 uppercase tracking-wider font-bold mb-1';

  const KPIResult = ({ label, value, unit, color, target }: any) => (
    <div className={`bg-transparent border ${color} rounded-xl p-4 flex flex-col gap-1`}>
      <p className='text-[11px] text-slate-500 uppercase tracking-widest font-bold'>{label}</p>
      <p className='text-xl font-bold text-white font-mono'>{value ?? '—'}{value ? <span className='text-sm text-slate-400 ml-1'>{unit}</span> : null}</p>
      {target && <p className='text-[11px] text-slate-600'>Target: {target}</p>}
    </div>
  );
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
        <KPIResult label='Service Level' value={serviceLevel} unit='%' color='border-transparent' target='≥ 80%' />
        <KPIResult label='Abandon Rate' value={abandonRate} unit='%' color='border-transparent' target='≤ 5%' />
        <KPIResult label='Avg Handle Time' value={aht} unit='min' color='border-transparent' target='≤ 4 min' />
        <KPIResult label='Utilization' value={utilization} unit='%' color='border-transparent' target='70–85%' />
        {csat && <KPIResult label='CSAT' value={csat} unit='%' color='border-transparent' target='≥ 85%' />}
