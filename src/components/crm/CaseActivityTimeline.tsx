  ...
  return (
    <div id="case-timeline" className="space-y-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-[11px] font-bold text-slate-300 uppercase tracking-wider">Activity History</h3>
      </div>

      {sortedEvents.length === 0 ? (
        <p className="text-[11px] text-slate-500 italic py-4">No activity logged.</p>
      ) : (
              ...
              <div className="bg-transparent border border-white/8 rounded-xl p-3 hover:bg-white/[0.06] transition-all">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 mt-1 font-medium whitespace-pre-wrap leading-relaxed">
                      {evt.description}
                    </p>
                  </div>
                  <span className="text-[11px] text-slate-500 font-sans flex items-center gap-1 shrink-0 bg-white/5 px-2 py-0.5 rounded-xl border border-white/8">
                ...
                {evt.details && (
                  <div className="mt-2 bg-white/[0.03] border border-white/8 p-2 rounded-xl text-[11px] font-sans text-slate-400 select-all max-h-32 overflow-y-auto">
                    {evt.details}
                  </div>
                )}
