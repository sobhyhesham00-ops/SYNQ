...
const InfoField = ({ label, value, copyable, mono }: { label: string; value: string; copyable?: boolean; mono?: boolean }) => (
  <div className="flex flex-col gap-0.5">
    <span className="text-[11px] text-slate-500 uppercase tracking-wider font-bold">{label}</span>
    <div className="flex items-center gap-1.5">
      <span className={`text-xs text-slate-100 font-medium ${mono ? 'font-mono' : ''}`}>{value || '—'}</span>
      {copyable && value && (
        <button
  ...
  if (!caseData) {
    return (
      <div id="case-drawer-empty-placeholder" className="h-full flex flex-col items-center justify-center p-8 bg-[#09090c] rounded-xl border border-white/8 text-center min-h-[500px]">
        ...
        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">No Case Selected</h3>
      ...
      <div className="bg-transparent border-b border-white/8 p-4 flex items-center justify-between z-10 shrink-0">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            <span className="text-[11px] uppercase font-bold tracking-widest text-[#2effc3] bg-[#00e3a5]/5 px-2 py-0.5 rounded-lg border border-[#00e3a5]/10 shrink-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            {caseData.status && (
              <span className={`px-3 py-1 rounded-lg text-[11px] font-bold uppercase tracking-wide inline-block max-w-full truncate ${['submitted', 'not_confirmed', 'awaiting_client_contact'].includes(caseData.status) ? 'bg-amber-500/10 text-amber-500' : caseData.status === 'tl_reviewing' ? 'bg-blue-500/10 text-blue-400' : ['sent_to_clinic', 'sent'].includes(caseData.status) ? 'bg-orange-500/10 text-orange-400' : ['in_progress', 'need_contact'].includes(caseData.status) ? 'bg-purple-500/10 text-purple-400' : ['answered', 'resolved', 'contacted'].includes(caseData.status) ? 'bg-emerald-500/10 text-emerald-400' : caseData.status === 'pending_tl' ? 'bg-yellow-500/10 text-yellow-400' : 'bg-slate-500/10 text-slate-400' }`}>
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="px-4 py-2 bg-transparent border border-white/12 text-white hover:bg-white/5 text-white rounded-xl text-xs font-bold flex items-center gap-2 transition-all cursor-pointer"
          >
        ...
        {[
          ...
          return (
            <button
              ...
              className={`py-3 flex items-center gap-1.5 transition-all cursor-pointer text-xs ${ isActive ? "border-b-2 border-indigo-500 text-white font-bold" : "text-slate-500 hover:text-slate-300" }`}
            >
              {t.icon}
        ...
        {activeTab === 'overview' && (
          <div className="space-y-5 animate-fade-in" id="drawer-tab-overview">
            ...
            <div className="p-4 bg-[#0a0a0e] rounded-xl border border-white/8">
              <p className="text-[11px] text-slate-500 uppercase tracking-wider font-bold mb-2">Case Details</p>
            ...
            {caseData.crmType === "inquiry" && isTLOreSupport && caseData.status === "submitted" && (
              <div className="bg-orange-500/10 border border-transparent text-orange-400 p-4 rounded-xl space-y-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold uppercase tracking-wider">Unanswered inquiry pending forward</span>
                </div>
                ...
                <button
                  ...
                  className="w-full bg-orange-500 hover:bg-orange-400 text-slate-950 font-bold text-[11px] uppercase tracking-wider py-2 rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                >
            ...
            {caseData.crmType === "tabby_tamara" && isTLOreSupport && caseData.status === "not_confirmed" && !showPartnerPanel && (
              <button
                ...
                className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-[11px] uppercase tracking-wider py-3 rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer border border-transparent"
              >
            ...
            {showPartnerPanel && (
              <div className="bg-emerald-500/10 border border-transparent p-4 rounded-xl space-y-3.5">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold text-emerald-400 uppercase tracking-widest">Fintech Forward Config</span>
                  <button onClick={() => setShowPartnerPanel(false)} className="text-slate-400 hover:text-white">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] uppercase tracking-wider text-slate-400 font-bold block">Partner Audit Comments</label>
                  <textarea
                    rows={3}
                    placeholder="Enter notes/resolutions to dispatch to partner or internal log history..."
                    value={partnerNotes}
                    onChange={(e) => setPartnerNotes(e.target.value)}
                    className="w-full bg-[#1b1b22] border border-white/8 rounded-xl p-2 text-[11px] text-white placeholder:text-slate-600 focus:outline-none focus:border-emerald-500"
                  />
                </div>
                <button
                  onClick={handlePartnerSubmit}
                  disabled={isSubmittingPartner}
                  className="w-full bg-emerald-500 hover:bg-emerald-400 disabled:bg-emerald-500/50 text-slate-950 font-bold text-[11px] uppercase tracking-wider py-2 rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                >
            ...
            {caseData.crmType === "complaint" && isTLOreSupport && caseData.status === "submitted" && !showResPanel && (
              <button
                ...
                className="w-full bg-rose-500 hover:bg-rose-400 text-slate-950 font-bold text-[11px] uppercase tracking-wider py-3 rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer border border-transparent"
              >
            ...
            {showResPanel && (
              <div className="bg-rose-500/10 border border-transparent p-4 rounded-xl space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold text-rose-400 uppercase tracking-widest">Complaint Solution Broker</span>
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] uppercase tracking-wider text-slate-400 font-bold block">Resolution Method</label>
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`py-1.5 px-2 rounded-xl text-[11px] font-semibold text-left transition-all flex items-center gap-1.5 ${ complaintResType === opt.value ? "bg-rose-500 text-slate-950 font-bold" : "bg-white/[0.03] hover:bg-white/[0.06] text-slate-300 border border-white/8" }`}
                      >
                        {opt.icon}
                        {opt.label}
                      </button>
                ...
                <div className="space-y-1">
                  <label className="text-[11px] uppercase tracking-wider text-slate-400 font-bold block">Resolution Instructions for Agent</label>
                  <textarea
                    rows={3}
                    ...
                    className="w-full bg-[#1b1b22] border border-white/8 rounded-xl p-2 text-[11px] text-white placeholder:text-slate-600 focus:outline-none focus:border-rose-500"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full bg-rose-500 hover:bg-rose-400 disabled:bg-rose-500/50 text-slate-950 font-bold text-[11px] uppercase tracking-wider py-2 rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                >
            ...
            {caseData.crmType === "complaint" && caseData.status === "need_contact" && (
              <div className="bg-transparent border border-white/8 p-4 rounded-xl space-y-2.5">
                <span className="text-[11px] uppercase tracking-wider text-slate-400 font-bold block">Contact Required Status</span>
                ...
                {caseData.raw.tlComment && (
                  <div className="bg-white/[0.02] border border-white/8 p-3 rounded-xl text-[11px] italic text-slate-400">
                    "{caseData.raw.tlComment}"
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-2.5 pt-1">
                  <button
                    ...
                    className="bg-transparent border border-white/12 text-white hover:bg-white/5 text-white font-bold text-[11px] uppercase tracking-wider py-1.5 px-3 rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    <Check className="w-3.5 h-3.5" /> Got Contact
                  </button>
                  <button
                    onClick={() => handleMarkContacted('attempted')}
                    className="bg-amber-600 hover:bg-amber-500 text-white font-bold text-[11px] uppercase tracking-wider py-1.5 px-3 rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                  >
            ...
            <div className="flex flex-wrap gap-2 pt-4 border-t border-white/8">
              {canEdit && (
                <button 
                  ...
                  className="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/8 text-slate-300 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
                >
                ...
                <>
                  {caseData.status !== "closed" ? (
                    <button
                      ...
                      className="px-4 py-2 bg-rose-500/10 hover:bg-rose-500/10 border border-transparent text-rose-400 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-amber-500/10 hover:bg-amber-500/10 border border-transparent text-amber-500 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer"
                    >
              ...
              {isTLOreSupport && (
                <button 
                  ...
                  className="px-4 py-2 bg-rose-500/10 hover:bg-rose-500/10 border border-transparent text-rose-400 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ml-auto"
                >
