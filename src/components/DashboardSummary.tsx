    ...
    <div className="space-y-6 w-full">
      ...
      {announcements && announcements.length > 0 && (() => {
        ...
        return (
          <motion.div 
            ...
            className="p-5 border border-transparent rounded-xl bg-gradient-to-r from-amber-500/15 via-yellow-500/5 to-transparent relative overflow-hidden group"
          >
            {/* Pulsing glow layer */}
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-start gap-3.5">
                ...
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 bg-amber-500/10 text-amber-300 rounded-xl border border-transparent">
                      Live Broadcast
                    </span>
                    ...
                    {latest.clinicFilter && latest.clinicFilter !== 'all' && (
                      <span className="text-[11px] font-sans px-1.5 py-0.5 bg-transparent border border-white/12 text-white text-indigo-300 rounded border border-transparent">
                        Target: {latest.clinicFilter}
                      </span>
                    )}
                  </div>
                  
                  <h4 className="text-xs font-bold text-slate-200 mt-1">Latest Update from Management:</h4>
                  <p className="text-slate-300 text-xs leading-relaxed max-w-4xl line-clamp-3">
                    {latest.message}
                  </p>
                </div>
              </div>
              ...
              <div className="shrink-0 flex items-center gap-3 self-end md:self-center">
                <button 
                  ...
                  className="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 active:scale-95 text-slate-950 font-bold text-[11px] uppercase tracking-widest rounded-xl transition-all flex items-center gap-2 cursor-pointer"
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-2">
        ...
        <motion.div 
          ...
          className="lg:col-span-2 p-6 rounded-xl bg-indigo-600 flex flex-col justify-between relative overflow-hidden group"
        >
          ...
          <div className="relative z-10 text-left">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center border border-white/15 text-white">
                 {isTL ? <ShieldCheck className="w-5 h-5" /> : <Users className="w-5 h-5" />}
              </div>
              <div>
                <p className="text-white/60 text-[11px] font-bold uppercase tracking-[0.2em]">Operational Overview</p>
                <h2 className="text-lg font-bold text-white tracking-tight">Salam, {(currentUser?.name || '').split(' ')[0]} 👋</h2>
                <p className="text-xs text-indigo-200 mt-1">{currentUser?.name}</p>
                ...
                {currentUser?.role === 'tl' && (
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-[11px] text-emerald-300 font-medium">Team Leader</span>
                  </div>
                )}

                {currentUser?.role === 'agent' && normalizeAgentLob(currentUser?.lob, currentUser?.role) && (
                  <div className="mt-2">
                    <span className="inline-flex items-center px-2 py-0.5 rounded-lg text-[11px] font-bold bg-transparent border border-white/12 text-white text-indigo-300 border border-transparent">
            ...
            <p className="text-indigo-100/80 text-xs max-w-md leading-relaxed mt-4">
              {isTL 
          ...
          <div className="relative z-10 flex gap-3 mt-6">
            <button 
              ...
              className="px-5 py-2 bg-white text-indigo-600 rounded-xl font-bold text-[11px] uppercase tracking-widest active:scale-95 transition-all flex items-center gap-2 cursor-pointer"
            >
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="px-3 py-1.5 bg-transparent border border-white/12 text-white text-emerald-400 rounded-lg text-[11px] font-bold uppercase tracking-widest">
              Active
            </div>
          </div>
          <div>
            <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest mb-1">Upcoming Rotation</p>
            <h3 className="text-lg font-bold text-white px-0.5">{myNextShift?.shiftLabel || 'Off Duty'}</h3>
            <p className="text-slate-500 text-xs mt-1 font-medium">{myNextShift?.date || 'No scheduled date'}</p>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="relative z-10">
            <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest mb-1">Queue Status</p>
        ...
        >
          ...
          <div className="relative z-10">
            <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest mb-1">QA Performance</p>
            <div className="flex items-baseline gap-2">
              <h3 className={`text-xl font-bold ${avgQaScore && avgQaScore < 70 ? 'text-red-400' : 'text-green-400'}`}>
        ...
        >
          ...
          <div className="relative z-10">
            <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest mb-1">Clinic Inquiries</p>
            <div className="flex items-baseline gap-2">
        ...
        >
          ...
          <div className="relative z-10">
            <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest mb-1">Tabby / Tamara Desk</p>
            <div className="flex items-baseline gap-2">
