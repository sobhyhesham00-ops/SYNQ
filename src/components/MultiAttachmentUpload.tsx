...
export const MultiAttachmentUpload: React.FC<MultiAttachmentUploadProps> = ({
  ...
  const slotsUsed = photos.length + uploadingFiles.length;
  const slotsLeft = maxFiles - slotsUsed;
  ...
  const handleFiles = (fileList: FileList | File[] | null) => {
    ...
    if (accepted.length < files.length) {
      toast.warning(`Only ${accepted.length} file(s) added — ${maxFiles} file limit reached.`);
    }
  ...
  const handleRemovePhoto = (index: number) => onPhotosChange(photos.filter((_, i) => i !== index));
  ...
  const isImageUrl = (url: string) => /\.(jpe?g|png|gif|webp)(\?|$)/i.test(url) || url.startsWith('data:image/') || url.includes('image%2F') || url.includes('image/');
    ...
    >
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <label className="text-[11px] font-bold uppercase tracking-widest text-slate-400">{photosLabel}</label>
          <span className="text-[11px] text-slate-500 font-mono">{slotsUsed}/{maxFiles}</span>
        </div>
        <div className="flex flex-wrap gap-2 items-start mt-2">
          {photos.map((photo, index) => {
            const isImg = isImageUrl(photo);
            ...
            return (
              <div key={index} className="relative group w-24 h-24 rounded-xl border border-white/8 overflow-hidden bg-white/[0.04]">
                {isImg ? (
                  <img referrerPolicy="no-referrer" src={photo} alt="attachment" className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex flex-col items-center justify-center bg-slate-800 gap-1 px-1.5 text-center">
                    <FileText className="w-6 h-6 text-slate-400" />
                    <span className="text-[9px] text-slate-400 font-bold uppercase truncate max-w-[80px]" title={fileName}>
                      {fileName}
                    </span>
                  </div>
                )}
          ...
          {slotsLeft > 0 && (
            <label className={`flex flex-col items-center justify-center w-24 h-24 border-2 border-dashed border-white/8 rounded-xl bg-white/5 transition-all ${!authReady ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/10 hover:border-transparent cursor-pointer group'}`}>
              <input type="file" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx" multiple className="hidden" disabled={!authReady} onChange={(e) => handleFiles(e.target.files)} />
        ...
        <p className="text-[10px] text-slate-500">Up to {maxFiles} files · 10MB each · images, PDF, Word, Excel</p>
      </div>
