...
// Replace P2PRequest creation (Wait, P2P is also a 'swap' technically. We didn't edit p2p. Let's see if P2P creates a swap)
content = content.replace(
...
// Replace Fintech creation (TabbyTamaraRequest)
content = content.replace(
...
// Replace Client Comms creation
content = content.replace(
  "screenshot: activeScreenshot || undefined\n      };",
  "screenshot: activeScreenshot || undefined,\n        photos: activePhotos,\n        links: activeLinks\n      };"
);

// Replace Cases creation
content = content.replace(
  "screenshot: activeScreenshot || undefined\n                              };",
  "screenshot: activeScreenshot || undefined,\n                                photos: activePhotos,\n                                links: activeLinks\n                              };"
);
...
// Replace JSX Tags
content = content.replace(
  /<ScreenshotUpload[\s\S]*?screenshot={swapScreenshot}[\s\S]*?onScreenshotChange={setSwapScreenshot}[\s\S]*?\/>/,
...
content = content.replace(
  /<ScreenshotUpload[\s\S]*?screenshot={annualScreenshot}[\s\S]*?onScreenshotChange={setAnnualScreenshot}[\s\S]*?\/>/,
...
content = content.replace(
  /<ScreenshotUpload[\s\S]*?screenshot={p2pScreenshot}[\s\S]*?onScreenshotChange={setP2pScreenshot}[\s\S]*?\/>/,
...
content = content.replace(
  /<ScreenshotUpload[\s\S]*?screenshot={activeScreenshot}[\s\S]*?onScreenshotChange={setActiveScreenshot}[\s\S]*?label="Payment \/ Identity Screenshot"[\s\S]*?\/>/,
...
content = content.replace(
  /<ScreenshotUpload[\s\S]*?screenshot={activeScreenshot}[\s\S]*?onScreenshotChange={setActiveScreenshot}[\s\S]*?label="Complaint Evidence \/ Screenshot"[\s\S]*?\/>/,
...
content = content.replace(
  /<ScreenshotUpload[\s\S]*?screenshot={activeScreenshot}[\s\S]*?onScreenshotChange={setActiveScreenshot}[\s\S]*?label="Customer Inquiry Screenshot"[\s\S]*?\/>/,
...
content = content.replace(
  /<ScreenshotUpload[\s\S]*?screenshot={activeScreenshot}[\s\S]*?onScreenshotChange={setActiveScreenshot}[\s\S]*?label="Evidence \/ Screenshot"[\s\S]*?\/>/,
