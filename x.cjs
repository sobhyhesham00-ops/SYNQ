...
const t = `                      {/* Inquiries Records display */}
...
const r = `                      {/* Inquiries Records display */}
                      {(() => {
...
if (code.includes(t)) {
  ...
  console.log("Success replacing t");
  
  // Now we need to close the IIFE 
  const t2 = `                                  </div>
                                );
                              })
                          )}
  ...
  if (code.includes(t2)) {
      ...
      console.log("Success replacing t2");
  } else {
      console.log("Failed to find t2... resorting to RegExp");
      // Let's use a regexp to replace the closing tags correctly
      const rx = /<\\/div>\\n\\s*\\);\\n\\s*\\}\\)\\n\\s*\\)\\}\\n\\s*<\\/div>\\n\\s*<\\/div>\\n\\s*<\\/div>\\n\\s*\\)}/m;
      if (rx.test(code)) {
          code = code.replace(rx, "</div>\\n                                );\\n                              })\\n                          )}\\n                        </div>\\n                      </div>\\n                      );\\n                    })()}\\n                    </div>\\n                  )}");
          console.log("Success with RegExp");
      } else {
        console.log("Failed all ways for t2");
      }
  }
} else {
